...
const val endMessage = "See you later!"
fun main() {
    val scanner = Scanner(System.`in`)
    do {
        println("Please enter the number of the desired habitat you wold like to view: ")
        when (scanner.nextLine().toString()) {
            "0" -> println(camel)
            "1" -> println(lion)
            "2" -> println(deer)
            "3" -> println(goose)
            "4" -> println(bat)
            "5" -> println(rabbit)
            "exit" -> println(endMessage)
        }
        println(endMessage)
    } while (!scanner.toString().contentEquals("exit"))
}